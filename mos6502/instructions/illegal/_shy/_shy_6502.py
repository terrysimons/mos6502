#!/usr/bin/env python3
"""SHY (SYA, SAY) instruction implementation for NMOS 6502 variants.

ILLEGAL INSTRUCTION - UNSTABLE

SHY stores Y & (high_byte + 1) to memory.

VARIANT: 6502 - Performs Y & (H+1) store
VARIANT: 6502A - Performs Y & (H+1) store
VARIANT: 6502C - Performs Y & (H+1) store
VARIANT: 65C02 - Acts as NOP (see _shy_65c02.py)
...
def shy_absolute_x_0x9c(cpu: "MOS6502CPU") -> None:
    ...
    effective_address: int = (base_address + int(cpu.X)) & 0xFFFF
