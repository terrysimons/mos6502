...
def write_c64gs_crt(path: Path, banks: list[bytes], name: str = "TEST") -> None:
    """Write a C64 Game System CRT file with multiple banks.

    C64GS cartridges use 8KB mode with bank switching via $DE00 writes.
    Reading from IO1 ($DE00-$DEFF) disables the cartridge until reset.
    ...
    header[0x14] = 1  # Version hi
    header[0x15] = 0  # Version lo
    header[0x16:0x18] = (15).to_bytes(2, 'big')  # Hardware type 15 = C64 Game System
    header[0x18] = 0  # EXROM = 0 (active)
    header[0x19] = 1  # GAME = 1 (inactive) -> 8KB mode
    name_bytes = name.encode('ascii')[:32].ljust(32, b'\x00')
    header[0x20:0x40] = name_bytes
...
def create_type15_test_cart(is_error_cart: bool = False, num_banks: int = 8) -> tuple[list[bytes], str]:
    """Create Type 15 (C64 Game System) test cartridge with bank switching tests.

    C64GS uses bank selection via $DE00 writes (bits 0-5, up to 64 banks).
    IMPORTANT: Reading from IO1 ($DE00-$DEFF) disables the cartridge!
    The test routine must avoid any reads from that region.

    Ref: https://vice-emu.sourceforge.io/
    ...
    # Display title and type info
    title = "TYPE 15 ERROR CART" if is_error_cart else "TYPE 15 VERIFY"
    code.extend(create_display_code(title, line=0, color=COLOR_WHITE))
    code.extend(create_display_code("TYPE: 15", line=1, color=COLOR_YELLOW))
    code.extend(create_display_code("NAME: C64 GAME SYSTEM", line=2, color=COLOR_YELLOW))
    code.extend(create_display_code(f"BANKS: {num_banks}", line=3, color=COLOR_YELLOW))
    ...
    # IMPORTANT: We MUST NOT read from $DE00-$DEFF as that disables the cart!
    # Only WRITE to $DE00 to switch banks.
    ram_routine = 0x0340
    test_routine = [
    ...
    code.extend(create_display_code("ALL TESTS PASSED", line=current_line, color=COLOR_GREEN))
    code.extend(create_display_code("TYPE 15 SUPPORTED: PASS", line=current_line + 1, color=COLOR_GREEN))
    code.extend([JMP_ABSOLUTE_0x4C, 0x00, 0x00])  # JMP to loop (placeholder)
    loop_jmp = len(code) - 2
    ...
    code.extend(create_display_code("VERIFICATION FAILED", line=current_line, color=COLOR_FAIL))
    code.extend(create_display_code("TYPE 15 SUPPORTED: FAIL", line=current_line + 1, color=COLOR_FAIL))
    ...
    desc = f"Type 15 verification ({num_banks} banks)" if not is_error_cart else f"Type 15 error cart ({num_banks} banks)"
    return banks, desc
...
def main():
    ...
    if GENERATE_ERROR_CARTS:
        ...
        # Type 15: C64 Game System error cart
        print("  Type 15 - C64 Game System error cart...")
        banks_err_v15, desc = create_type15_test_cart(is_error_cart=True, num_banks=8)
        path = error_cart_dir / "error_cart_type_15_c64_game_system.bin"
        with open(path, 'wb') as f:
            for bank in banks_err_v15:
                f.write(bank)
        print(f"    {path.name}")
        ...
        # Type 19: Magic Desk error cart
        print("  Type 19 - Magic Desk error cart...")
        banks_err_v19, desc = create_type19_test_cart(is_error_cart=True, num_banks=8)
        path = error_cart_dir / "error_cart_type_19_magic_desk.bin"
        with open(path, 'wb') as f:
            for bank in banks_err_v19:
                f.write(bank)
        print(f"    {path.name}")
    ...
    path_v15_crt = cartridge_types_dir / "test_cart_type_15_c64_game_system_system_3.crt"
    write_c64gs_crt(path_v15_crt, banks_v15, name="TYPE 15 TEST")
    print(f"    {path_v15_crt.name} ({desc_v15})")
